...
import {convertStringToEnum} from "./TransactionType";
...
export class TransactionService {
  ...
  getTransactionsByYearAndMonth(year: number, month: number): Observable<Transaction[]> {
    ...
    const response = this.http.get<Transaction[]>(url);
    response.subscribe((transaction)=>{
      transaction.forEach(transaction => transaction.type = convertStringToEnum(transaction.type));
    })
    return response;
  }
